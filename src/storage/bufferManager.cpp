...
Page BufferManager::getFromPool(string pageName)
{
    ...
    for (auto page : this->pages)
        if (pageName == page.pageName)
            return page;
    // Should not reach here - caller should check inPool first
    return Page(); // Return empty page to avoid undefined behavior
}
...
void BufferManager::writePage(string tableName, int pageIndex, vector<vector<int>> rows, int rowCount)
{
    ...
    // Invalidate pool entry so next getPage() reads the updated page from disk
    string pageName = "data/temp/" + tableName + "_Page" + to_string(pageIndex);
...
void BufferManager::resetPool()
{
    logger.log("BufferManager::resetPool");
    this->pages.clear();
}
